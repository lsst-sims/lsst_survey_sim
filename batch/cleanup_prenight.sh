...
emit_work_dir_cleanup() {
    ...
    local WORK_DIR WORK_DIR_NAME VENV_TARGET VENV_NAME PACK_FILE IDX_FILE REV_FILE
    for WORK_DIR in "${WORK_ROOT}"/*/; do
        ...
        # Emit removal of expanded pack/index files, then remove work directory
        {
            echo "# Remove work directory: ${WORK_ROOT}/${WORK_DIR_NAME}"
        } >> "${OUTPUT_SCRIPT}"

        while IFS= read -r -d '' PACK_FILE; do
            echo "rm -f --one-file-system ${PACK_FILE}" >> "${OUTPUT_SCRIPT}"
        done < <(find "${WORK_DIR}/ts_config_scheduler/.git/objects/pack" -maxdepth 1 -type f -name "pack-*.pack" -print0 2>/dev/null)

        while IFS= read -r -d '' IDX_FILE; do
            echo "rm -f --one-file-system ${IDX_FILE}" >> "${OUTPUT_SCRIPT}"
        done < <(find "${WORK_DIR}/ts_config_scheduler/.git/objects/pack" -maxdepth 1 -type f -name "pack-*.idx" -print0 2>/dev/null)

        while IFS= read -r -d '' REV_FILE; do
            echo "rm -f --one-file-system ${REV_FILE}" >> "${OUTPUT_SCRIPT}"
        done < <(find "${WORK_DIR}/ts_config_scheduler/.git/objects/pack" -maxdepth 1 -type f -name "pack-*.rev" -print0 2>/dev/null)
...
echo "Review it with: less ${OUTPUT_SCRIPT}"
